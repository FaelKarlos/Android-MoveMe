...
public class HttpServiceAdministrador extends AsyncTask<String, Void, Administrador> {

    private String administrador;
    private String operacao;

//Construtor com um parâmetro, que nesse caso é o motorista a ser inserido
    public HttpServiceAdministrador(String administrador, String operacao) {
        this.administrador = administrador;
        this.operacao = operacao;
    }

    public HttpServiceAdministrador() {

    }

    @Override
    protected Administrador doInBackground(String... voids) {
        StringBuilder resposta = new StringBuilder();
        System.out.println("Operação passada por parametro: " + operacao);
        Gson gson = new Gson();

        //Converte o motorista passado para Json
        String jsonUsuario = gson.toJson(administrador);

        Administrador motoristaRetorno = gson.fromJson(administrador, Administrador.class);
        ...
        try {
        //Localização do web service
        String jsonUsuarioLogin = gson.toJson(administrador);

            Administrador motoristaLogin = gson.fromJson(administrador, Administrador.class);

        System.out.println("Usuario passado: " + motoristaLogin.toString());

        URL url = new URL("http://b8439639.ngrok.io/MoveMe/rest/administrador/" + motoristaLogin.getNome() + "/");
        HttpURLConnection connection = (HttpURLConnection) url.openConnection();
        ...
        motoristaRetorno = gson.fromJson(str, Administrador.class);
        } catch (MalformedURLException e) {
        e.printStackTrace();
        } catch (IOException e) {
        e.printStackTrace();
        ...
        String jsonUsuarioLogin = gson.toJson(administrador);
            ...
            Administrador passageiroResetaSenha = gson.fromJson(administrador, Administrador.class);

        System.out.println("Usuario passado: " + passageiroResetaSenha.toString());

        URL url = new URL("http://b8439639.ngrok.io/MoveMe/rest/administrador/resuperarsenha/" + passageiroResetaSenha.getEmail() + "/");
        HttpURLConnection connection = (HttpURLConnection) url.openConnection();
        connection.setRequestMethod("GET");
